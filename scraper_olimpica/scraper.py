...
class OlimpicaScraper:
    def __init__(self, output_file="../data/productos_olimpica.csv"):
    ...
    def fetch_products(self, category_path, default_category):
        print(f"Scraping category: {default_category}...")
        step = 49
        _from = 0
        _to = step
        total_fetched = 0
        ...
        while True:
            # Olimpica uses VTEX Legacy Search API
            url = f"https://www.olimpica.com/api/catalog_system/pub/products/search{category_path}?_from={_from}&_to={_to}"
            
            try:
                # HTTP ONLY usando Scrapling
                ...
                for node in products:
                    product_data = self._extract_product_data(node, default_category)
                    if product_data:
                        results.append(product_data)
                
                total_fetched += len(products)
                print(f"[{default_category}] Fetched {total_fetched} products so far...")
                ...
                _from += step + 1
                _to += step + 1
                time.sleep(3) # Increase delay to 3 seconds to avoid 429s
    ...
    def _extract_product_data(self, node, default_category):
        try:
            ...
            if categories and len(categories) > 0:
                # categories are like "/Supermercado/Licores/Cervezas/"
            ...
            # REGLA ESTRICITA AGENTS.MD: Clasificación Dinámica de Tipo de Producto
    ...
    def save_to_json(self, all_products, json_file="../data/productos_olimpica.json"):
        import os
...
if __name__ == '__main__':
    scraper = OlimpicaScraper(output_file="../data/productos_olimpica.csv")
    ...
    for path, name in olimpica_paths:
        all_products.extend(scraper.fetch_products(path, name))
